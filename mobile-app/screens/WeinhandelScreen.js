...
import { 
  getVerifiedWinehandel, 
  subscribeVerifiedWinehandel,
...
export default function WeinhandelScreen({ onNavigate, onLogout, isAdmin = false, unreadCount = 0, chats = [], isLoggedIn = false, wishlistMatchCount = 0 }, isPro = false) {
  const [winehandel, setWinehandel] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isMenuVisible, setIsMenuVisible] = useState(false);
  const [selectedWinehandel, setSelectedWinehandel] = useState(null);
  ...
  // Filter-Funktion für Weinhandel
  const filterWinehandel = (winehandelList, searchQuery) => {
    let filtered = winehandelList;
    
    // Filtere nach Suchtext
    if (searchQuery && searchQuery.trim() !== '') {
      const query = searchQuery.toLowerCase().trim();
      filtered = filtered.filter(winehandelItem => {
        const name = winehandelItem.name?.toLowerCase() || '';
        const description = winehandelItem.description?.toLowerCase() || '';
        const address = winehandelItem.address?.toLowerCase() || '';
        const region = winehandelItem.region?.toLowerCase() || '';
  ...
  // Gefilterte Weinhandel-Unternehmen
  const filteredWinehandel = filterWinehandel(winehandel, searchText);
  ...
  const loadWinehandel = async () => {
    try {
      setIsLoading(true);
      const winehandelData = await getVerifiedWinehandel();
      setWinehandel(winehandelData);
    } catch (error) {
  ...
  useEffect(() => {
    const unsubscribe = subscribeVerifiedWinehandel((winehandelData) => {
      setWinehandel(winehandelData);
      setIsLoading(false);
    });
  ...
  const handleWinehandelPress = (winehandelItem) => {
    setSelectedWinehandel(winehandelItem);
  ...
  const handleCloseModal = () => {
    ...
    setSelectedWinehandel(null);
    setCurrentImageIndex(0);
  };

  const handleImageSwipe = (direction) => {
    if (!selectedWinehandel || !selectedWinehandel.images || selectedWinehandel.images.length === 0) return;
    
    if (direction === 'left') {
      setCurrentImageIndex((prev) => 
        prev < selectedWinehandel.images.length - 1 ? prev + 1 : 0
      );
    } else {
      setCurrentImageIndex((prev) => 
        prev > 0 ? prev - 1 : selectedWinehandel.images.length - 1
      );
    }
  };
...
{/* Header mit Überschrift */}
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            <Text style={styles.greeting}>Weinhandel</Text>
          </View>
        ...
        {viewMode === 'container' ? (
            ...
            <View style={styles.dashboardContainer}>
              {isLoading ? (
                <View style={styles.loadingState}>
                  ...
                  <Text style={styles.loadingText}>Weinhandel-Unternehmen werden geladen...</Text>
                </View>
              ) : filteredWinehandel.length === 0 ? (
                <View style={styles.emptyState}>
                  <Text style={styles.emptyIcon}>🍷</Text>
                  <Text style={styles.emptyTitle}>
                    {searchText ? 'Keine Weinhandel-Unternehmen gefunden' : 'Keine Weinhandel-Unternehmen verfügbar'}
                  </Text>
                ...
                <View style={styles.winehandelList}>
                  {filteredWinehandel.map((winehandelItem, index) => (
                    <TouchableOpacity
                      key={winehandelItem.id}
                      style={styles.winehandelCard}
                      onPress={() => handleWinehandelPress(winehandelItem)}
                    >
                      {winehandelItem.images && winehandelItem.images.length > 0 ? (
                        <OptimizedImage
                          source={{ uri: winehandelItem.images[0] }}
                          style={styles.winehandelCardImage}
                          resizeMode="cover"
                        />
                      ) : (
                        <View style={styles.winehandelCardPlaceholder}>
                          <Text style={styles.winehandelCardPlaceholderText}>🍷</Text>
                        </View>
                      )}
                      <View style={styles.winehandelCardInfo}>
                        <Text style={styles.winehandelCardName} numberOfLines={2}>
                          {winehandelItem.name}
                        </Text>
                        {winehandelItem.region && (
                          <Text style={styles.winehandelCardRegion} numberOfLines={1}>
                            {winehandelItem.region}
                          </Text>
                        )}
                      </View>
          ...
          <View style={styles.content}>
            <FlatList
              data={filteredWinehandel}
              keyExtractor={(item) => item.id}
              renderItem={({ item: winehandelItem }) => (
                <TouchableOpacity
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            {/* Bild-Slider */}
            {selectedWinehandel && selectedWinehandel.images && selectedWinehandel.images.length > 0 && (
              <View style={styles.modalImageContainer}>
                ...
                >
                  {selectedWinehandel.images.map((image, index) => (
                ...
                {/* Bild-Indikatoren */}
                {selectedWinehandel.images.length > 1 && (
                  <View style={styles.imageIndicatorContainer}>
                    {selectedWinehandel.images.map((_, index) => (
                      <View
                        key={index}
                        style={[
            ...
            <ScrollView style={styles.modalInfoContainer} showsVerticalScrollIndicator={false}>
              <Text style={styles.modalTitle}>{selectedWinehandel?.name}</Text>
              
              {selectedWinehandel?.region && (
                <View style={styles.modalInfoRow}>
                  <Text style={styles.modalInfoLabel}>📍 Region:</Text>
                  <Text style={styles.modalInfoValue}>{selectedWinehandel.region}</Text>
                </View>
              )}
              ...
              {selectedWinehandel?.address && (
                <View style={styles.modalInfoRow}>
                  ...
                  <Text style={styles.modalInfoValue}>{selectedWinehandel.address}</Text>
                </View>
              )}
              
              {selectedWinehandel?.website && (
                <View style={styles.modalInfoRow}>
                  <Text style={styles.modalInfoLabel}>🌐 Website:</Text>
                  <Text style={styles.modalInfoValue}>{selectedWinehandel.website}</Text>
                </View>
              )}
              
              {selectedWinehandel?.contactEmail && (
                <View style={styles.modalInfoRow}>
                  <Text style={styles.modalInfoLabel}>📧 E-Mail:</Text>
                  <Text style={styles.modalInfoValue}>{selectedWinehandel.contactEmail}</Text>
                </View>
              )}
              
              {selectedWinehandel?.phone && (
                <View style={styles.modalInfoRow}>
                  <Text style={styles.modalInfoLabel}>📞 Telefon:</Text>
                  <Text style={styles.modalInfoValue}>{selectedWinehandel.phone}</Text>
                </View>
              )}
